"""
MBT (Maxilia's Bill Tracker) Cog — track NJ Legislature bills and send @everyone alerts on updates.
...
Scheduler:
  Poll times are configured in configs/scheduler_config.ini.
  Changes to that file take effect on the next heartbeat (no restart needed).
"""
...
class BillsCog(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot) -> None:
        ...
        # Scheduler — hot-reloads config/scheduler_config.ini on every heartbeat
        self._scheduler = SchedulerConfig(_SCHEDULER_CONFIG_PATH)
        self._last_poll_time: datetime.datetime | None = None

        self._poll.start()
        log.info("MBT cog started — using time-based scheduler (%s)", _SCHEDULER_CONFIG_PATH)
    ...
    async def cog_check(self, ctx: commands.Context) -> bool:
        ...
        if ctx.channel.id != self.channel_id:
            log.debug(
                "MBT command '%s' from %s ignored — wrong channel (%d)",
    ...
    # ------------------------------------------------------------------
    # Background poll — 1-minute heartbeat with time-based gating
    # ------------------------------------------------------------------

    @tasks.loop(minutes=1)
    async def _poll(self) -> None:
        """Heartbeat loop — fires _do_poll() only at configured scheduled times."""
        now = datetime.datetime.now(tz=self._scheduler.timezone).replace(second=0, microsecond=0)
        scheduled = self._scheduler.get_scheduled_times()

        if not scheduled:
            log.debug("Poll heartbeat: no scheduled times for today — skipping")
            return

        if now.time() not in scheduled:
            return

        if self._last_poll_time == now:
            return  # guard against drift double-fire within the same minute

        self._last_poll_time = now
        log.info("Poll heartbeat: firing at %s", now.strftime("%Y-%m-%d %H:%M %Z"))
        await self._do_poll()
